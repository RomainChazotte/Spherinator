...
class ZernikeEncoderAuto(pl.LightningModule):
    '''
    ...
    def __init__(self, n_in, n_output, in_channels, num_channels, device,Very_small=True,Very_large=True, image_size=14, p_dropout=0.5, num_classes=10):
        ...
        # self.SkipLinear_2 = MultiLinear(int(num_channels/8), int(num_channels/4), size, non_lin=True)

        size = self.Product01._calc_size(int(n_temp/4))
        self.Rnorm03 = nn.RMSNorm([size,2], elementwise_affine=False)
        # self.SkipLinear_3 = MultiLinear(int(num_channels/4), int(num_channels/2), size, non_lin=True)

        size = self.Product01._calc_size(int(n_temp/8))
        self.Rnorm04 = nn.RMSNorm([size,2], elementwise_affine=False)
        # self.SkipLinear_4 = MultiLinear(int(num_channels/2), int(num_channels), size, non_lin=True)


        if self.Very_small:
            size = self.Product01._calc_size(int(n_temp/16))
            # self.SkipLinear_5 = MultiLinear(int(num_channels), int(num_channels), size, non_lin=True)
        self.Rnorm05 = nn.RMSNorm([size,2], elementwise_affine=False)

        self.MultiLinear_1 = MultiLinear(int(num_channels), int(num_channels), size, non_lin=True)
        if self.Very_large:
            self.MultiLinear_2 = MultiLinear(int(num_channels), int(num_channels/16), size, non_lin=True)
            self.MultiLinear_0 = MultiLinear(int(num_channels/16), 1, size, non_lin=False)
        else:
            self.MultiLinear_2 = MultiLinear(int(num_channels), 1, size, non_lin=False)
    ...
    def forward(self, x) -> torch.tensor:
        ...
        if self.Very_large:
            x = self.Product00(x,x)#+self.SkipLinear_0(x)
            x = self.Rnorm00(x)

        x = self.Product01(x,x)#+self.SkipLinear_1(x[:,:,self.Product01.out_mask])
        x = self.Rnorm01(x)

        # Second Zernike transformation
        # breakpoint()
        x = self.Product02(x,x)#+self.SkipLinear_2(x[:,:,self.Product02.out_mask])
        x = self.Rnorm02(x)

        x = self.Product03(x,x)#+self.SkipLinear_3(x[:,:,self.Product03.out_mask])
        x = self.Rnorm03(x)

        # x = self.Product04(x,x)+self.SkipLinear_4(x[:,:,self.Product04.out_mask])
        # x = self.Rnorm04(x)
        x = self.Product04(x,x)#+self.SkipLinear_4(x[:,:,self.Product04.out_mask])
        x = self.Rnorm04(x)

        if self.Very_small:
            x = self.Product05(x,x)#+self.SkipLinear_5(x[:,:,self.Product05.out_mask])
            x = self.Rnorm05(x)
