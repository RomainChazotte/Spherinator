...
a = Radial_function_no_zero(1,1,5,1)
#print(a)
b = Radial_function_no_zero(3,1,5,1)
#print(b)
c = Multiply(a,b,3)
#print(c)
c = np.array([12.,-8.])
...
def forward(m1,m2,n1,n2,n_max):
    ...
    #print(len(out1))
    #print(len(out2))
    out = np.zeros(lower,dtype=float)
    out = np.append(out,out1)
    out = np.append(out,np.zeros(inbetween))
    if not m_out1 == m_out2:
        out = np.append(out,out2)
    out = np.append(out,np.zeros(higher))
    #print(len(out))
    #dackel
...
def Zernicke_matrix_generator(n_max):
    ...
    grid = np.zeros((lengh,lengh,lengh))
...
one[1]=1
two[16]=1
print(np.einsum('ijk,i,j->k',grid,one,two))
print(np.shape(grid))
